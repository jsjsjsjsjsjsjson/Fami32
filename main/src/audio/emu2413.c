...
/* KSL + TL Table */
/*
 * These two tables are large (tllTable ~= 128 KiB, dphaseTable ~= 256 KiB).
 * Keeping them as static arrays puts them into .dram0.bss on ESP32 and can make
 * the firmware fail at link time.  Allocate them lazily instead; with PSRAM
 * enabled we explicitly prefer external RAM.
 */
static uint32 (*tllTable)[8][1 << TL_BITS][4] = NULL;
static int32 rksTable[2][8][2];

/* Phase incr table for PG */
static uint32 (*dphaseTable)[8][16] = NULL;

// Added by jsr
int32 opll_volumes[10];

static void *
opll_table_calloc (size_t nmemb, size_t size)
{
#ifdef ESP_PLATFORM
#ifdef CONFIG_SPIRAM
...
static int
opll_ensure_tables_allocated (void)
{
  if (tllTable != NULL && dphaseTable != NULL)
    return 1;

  if (tllTable == NULL)
    tllTable = (uint32 (*)[8][1 << TL_BITS][4]) opll_table_calloc (16, sizeof (*tllTable));

  if (dphaseTable == NULL)
    dphaseTable = (uint32 (*)[8][16]) opll_table_calloc (512, sizeof (*dphaseTable));
...
{
  if (!opll_ensure_tables_allocated ())
    return 0;
